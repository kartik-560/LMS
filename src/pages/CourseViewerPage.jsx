...
useEffect(() => {
  ...
  async function loadQuizForChapter(chapterId) {
    ...
    try {
      ...
      let full = first;
      if (!first.questions) {
        const fullRes = await axios.get(`${API_BASE}/api/assessments/${first.id}`, {
          headers: makeHeaders(),
        });
        full = fullRes.data;
      }
          ...
          <div className="space-y-3">
            {chapter.attachments.map((url, index) => (
              ...
              >
                ...
                View PDF Attachment {chapter.attachments.length > 1 ? index + 1 : ""}
              </a>
            ))}
          </div>
        </div>
    ...
    return (
      <div className="border rounded-lg p-4">
        ...
        <div className="space-y-2">
          {q.options.map((opt, i) => (
            <label key={i} className="flex items-center space-x-2">
              <input type="checkbox" disabled={disabled} checked={arr.includes(i)} onChange={() => toggle(i)} />
              <span>{opt}</span>
            </label>
          ))}
        </div>
